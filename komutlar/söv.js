exports.run = async (client, msg, args) => {
 ...
 description: (':no_entry_sign: Ya geçerli birini etiketle ya da sana sövmemi istiyosan kendini etiketle.')
  ...
  if(member.id === "676468301433536512")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (':no_entry_sign: Yarrak Sen kimin Sahibine Sovuyon')
}})
  if(member.id === "452093350921764874")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Birşeyler ters gitti @✧ hi'm sad#4415 etiketlemişsin yarram!`)
}})
  if(member.id === "676468301433536512")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Orda durucan  @⊹ Hi I'm Sad#6858 Yapımcım sövemezsin amını sikerim.`)
}})
   if(member.id === "758609557307129906")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Orda durucan  Onu Etiketlersen Bidaha Gotunu Sikerim`)
}})
